...
class ConfigureNetwork(NosDeviceAction):
    ...
    def run(self, switch_address, switch_username, switch_password, vlan):
        ...
        changes = {}
        with self.mgr(conn=self.conn, auth=self.auth,hostkey_verify=False) as device:
            self.logger.info('successfully connected to %s', self.host)
            changes['create_network'] = self._configure_network(device,vlan)
            self.logger.info('closing connection to %s -- all done!', self.host)
        return changes

    def _configure_network(self, device, vlan):
        """Create L2 Network on device
        """
